...
from models import BertForSpanMarkerNER8, AlbertForSpanMarkerNER4
    ...
    def __getitem__(self, idx):
        ...
        if self.model_type not in ['bertspan', 'robertaspan', 'albertspan']:
            ...
            attention_mask = torch.zeros(
                ( self.max_seq_length + self.max_entity_length +self.max_entity_length, self.max_seq_length + self.max_entity_length + self.max_entity_length ),
                dtype=torch.int64)
            attention_mask[:L, :L] = 1
            position_ids = list(range(position_plus_pad, position_plus_pad + self.max_seq_length)) + [
                0] * self.max_entity_length * 4
...
def train(args, model, tokenizer):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    if args.evaluate_during_training:  # Only evaluate when single GPU otherwise metrics may not average well
                        ...
                        r1 = results['recall']
                        tb_writer.add_scalar('r1', r1, global_step)
    ...
    return global_step, tr_loss / global_step, best_r1
...
def main():
    ...
    if args.do_train and args.local_rank in [-1, 0] and args.output_dir.find('test') == -1:
        create_exp_dir(args.output_dir,
                       scripts_to_save=['run_ner8.py', 'models.py',])
    ...
    if args.model_type.startswith('albert'):
        config_class, model_class, tokenizer_class = AlbertConfig, AlbertForSpanMarkerNER4, AlbertTokenizer
    else:
        config_class, model_class, tokenizer_class = BertConfig, BertForSpanMarkerNER8, BertTokenizer
    ...
    # Training
    if args.do_train:
        global_step, tr_loss, best_r1 = train(args, model, tokenizer)

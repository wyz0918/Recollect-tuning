...
from models import BertForSpanMarkerNER6, AlbertForSpanMarkerNER6
...
class ACEDatasetNER(Dataset):
    ...
    def initialize(self):
        ...
        logger.info(len(self.ner_golden_labels))  # 334
        # logger.info(len(self.tot_recall), self.tot_recall)
    ...
    def __getitem__(self, idx):
        ...
        if self.model_type not in ['bertspan', 'robertaspan', 'albertspan']:
            ...
            position_ids = list(range(position_plus_pad, position_plus_pad + self.max_seq_length)) + [
                0] * self.max_entity_length +  [0] * self.max_entity_length +  [0] * self.max_entity_length
...
def main():
    ...
    if args.do_train and args.local_rank in [-1, 0] and args.output_dir.find('test') == -1:
        create_exp_dir(args.output_dir,
                       scripts_to_save=['run_ner6.py', 'models.py',])
    ...
    if args.model_type.startswith('albert'):
        config_class, model_class, tokenizer_class = AlbertConfig, AlbertForSpanMarkerNER6, AlbertTokenizer
    else:
        config_class, model_class, tokenizer_class = BertConfig, BertForSpanMarkerNER6, BertTokenizer

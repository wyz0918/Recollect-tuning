...
from models import BertForSpanMarkerNER, AlbertForSpanMarkerNER, BertForBinarySpanMarkerNER, AlbertForBinarySpanMarkerNER
...
class ACEDatasetNER(Dataset):
    def __init__(self, tokenizer, args=None, evaluate=False, do_test=False):
        ...
        # self.ner_label_list = ['NIL', 'ENTITY','START']
    ...
    def initialize(self):
        ...
        for l_idx, line in enumerate(f):
            ...
            for n in range(len(subword_sentence_boundaries) - 1):
                ...
                for start, end, label in sentence_ners:
                    # entity_labels[token2subword[start]-1] = 2
                    # entity_labels[token2subword[start]] = 1
                    # self.ner_golden_labels.add(((l_idx, n), start, 'ENTITY'))

                    entity_start_labels[token2subword[start]] = ner_label_map[label]
                    entity_end_labels[token2subword[end + 1]] = ner_label_map[label]

                    # pre_ners += [token2subword[start]]
                ...
                for entity_start in range(left_context_length, left_context_length + sentence_length):
                    ...
                    if doc_entity_start not in subword_start_positions:
                        ...
                        label_start = entity_start_labels.get(doc_entity_start, 0)
                        entity_start_labels.pop(doc_entity_start, None)
                        entity_start_infos.append((entity_start + 1, label_start,(subword2token[doc_entity_start])))

                        label_end = entity_end_labels.get(doc_entity_start, 0)
                        entity_end_labels.pop(doc_entity_start, None)
                        entity_end_infos.append((entity_start + 1, label_end, (subword2token[doc_entity_start])))

                item = {
                    'sentence': target_tokens,
                    'examples_start': entity_start_infos,
                    'examples_end': entity_end_infos,
                    'example_index': (l_idx, n),
                    # 'example_L': len(entity_infos)
                }
                self.data.append(item)
                #
                #     # print('subword2token[doc_entity_start]',subword2token[doc_entity_start])
                #
                #     # if words[subword2token[doc_entity_start]] not in nounwords:
                #     #     continue
                #     # tw_words = pos_tag(words)
                #
                ...
                #             continue
                #
                #         # tw_start =tw_words[subword2token[doc_entity_start]][1]
                #         # tw_end = tw_words[subword2token[doc_entity_end-1]][1]
                #
                #
                ...
                # print('entity_infos',entity_infos)
                #
                # item = {
                #     'sentence': target_tokens,
                #     'examples': entity_infos,
                #     'example_index': (l_idx, n),
                #     # 'example_L': len(entity_infos)
                # }
                # self.data.append(item)
        ...
        # logger.info('maxEL: %d', maxEL)
        # logger.info('maxEL: %d', maxSuTokenL)
    ...
    def __getitem__(self, idx):
        ...
        #         input_ids = input_ids + [30000] * L + [0] * (
        #                     self.max_pair_length - L)
        #         input_ids = input_ids + [30001] * (L) + [0] * (
        #                     self.max_pair_length - L)
        #     elif self.model_type.startswith('roberta'):
        #         input_ids = input_ids + [50261] * L + [0] * (
        #                     self.max_pair_length - L)
        #         input_ids = input_ids + [50262] * L + [0] * (
        #                     self.max_pair_length - L)
        #     else:
        #         input_ids = input_ids + [1] * L + [0] * (
        #                     self.max_pair_length - L)
        #         input_ids = input_ids + [2] * L + [0] * (
        #                     self.max_pair_length - L)
        ...
        #     (self.max_seq_length + L, self.max_seq_length + L),
        #     dtype=torch.int64)
        # attention_mask[:L, :L] = 1
        # position_ids = list(range(position_plus_pad, position_plus_pad + self.max_seq_length)) + list(
        #     range(position_plus_pad, position_plus_pad + self.max_seq_length))
        ...
        input_mask = [1] * L + [0] * (self.max_seq_length - L)
        id_start = []
        id_end = []
        for x_idx, x in enumerate(entry['examples_start']):
            m1 = x[0]
            label_start = x[1]
            mentions_start.append(x[2])
            mention_pos_start.append(m1)
            labels_start.append(label_start)

        for x_idx, x in enumerate(entry['examples_end']):
            m1 = x[0]
            label_end = x[1]
            mentions_end.append(x[2])
            mention_pos_end.append(m1)
        ...
        #
        #
        #     # id_start += [input_ids[m1[0]]]
        #     # id_end += [input_ids[m1[1]]]
        #     #
        #     #
        #     #
        #     #
        #     # position_ids[w3] = m1[0]
        #     # position_ids[w4] = m1[1]
        #
        #     for xx in [w1]:
        #         input_mask[xx] = 2
        #
        ...
        #                 self.max_pair_length - L)+ id_end +[0] * (
        #                 self.max_pair_length - len(entry['examples']))
        ...
        labels_start += [-1] * (num_pair - len(labels_start))
        labels_end += [-1] * (num_pair - len(labels_end))


        mention_pos_start += [0] * (num_pair - len(mention_pos_start))
        mention_pos_end += [0] * (num_pair - len(mention_pos_end))

        # boundary_width += [0] * (num_pair - len(boundary_width))

        # print('input_ids_shape:',len(labels_start))
        # print('attention_mask_shape:',len(mention_pos_start))
        # print('position_ids_shape:',len(labels_end))
        # print('attention_mask_shape:',len(mention_pos_end))


        item = [torch.tensor(input_ids),
                torch.tensor(attention_mask),
                torch.tensor(position_ids),
                torch.tensor(labels_start, dtype=torch.int64),
                torch.tensor(labels_end, dtype=torch.int64),
                torch.tensor(mention_pos_start),
                torch.tensor(mention_pos_end),
                # torch.tensor(full_attention_mask)
                ]
        # print(mentions_start)
        # print(mentions_end)

        if self.evaluate:
            item.append(entry['example_index'])
            item.append(mentions_start)
            item.append(mentions_end)
...
def train(args, model, tokenizer):
    ...
    tr_ner_loss1, logging_ner_loss1 = 0.0, 0.0
    tr_ner_loss2, logging_ner_loss2 = 0.0, 0.0
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            inputs = {'input_ids': batch[0],
                      ...
                      'labels_start': batch[3],
                      'labels_end': batch[4],
                      'mention_pos_start':batch[5],
                      'mention_pos_end': batch[6],
                      }
            ...
            if args.gradient_accumulation_steps > 1:
                ...
                ner_loss1 = ner_loss1 / args.gradient_accumulation_steps
                ner_loss2 = ner_loss2 / args.gradient_accumulation_steps
            ...
            tr_loss += loss.item()
            if ner_loss1 > 0:
                tr_ner_loss1 += ner_loss1.item()
            if ner_loss2 > 0:
                tr_ner_loss2 += ner_loss2.item()
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    tb_writer.add_scalar('RE_loss', (tr_ner_loss1 - logging_ner_loss1) / args.logging_steps, global_step)
                    logging_ner_loss1 = tr_ner_loss1

                    tb_writer.add_scalar('NER_loss', (tr_ner_loss2 - logging_ner_loss2) / args.logging_steps, global_step)
                    logging_ner_loss2 = tr_ner_loss2
...
def evaluate(args, model, tokenizer, prefix="", do_test=False):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        indexs = batch[-3]
        batch_m2s_start = batch[-2]
        batch_m2s_end = batch[-1]
        ...
        with torch.no_grad():
            ...
            ner_logits1 = outputs[0]
            ner_logits1 = torch.nn.functional.softmax(ner_logits1, dim=-1)
            ner_values1, ner_preds1 = torch.max(ner_logits1, dim=-1)

            ner_logits2 = outputs[1]
            ner_logits2 = torch.nn.functional.softmax(ner_logits2, dim=-1)
            ner_values2, ner_preds2 = torch.max(ner_logits2, dim=-1)



            for i in range(len(indexs)):
                index = indexs[i]
                m2s_start = batch_m2s_start[i]
                m2s_end = batch_m2s_end[i]
                for j in range(len(m2s_start)):
                    start = m2s_start[j]
                    ner_label_start = eval_dataset.ner_label_list[ner_preds1[i, j]]
                    if ner_label_start != 'NIL':
                        scores_start.append((float(ner_values1[i, j]), ner_label_start,start))
                for k in range(len(m2s_end)):
                    end = m2s_end[k]
                    ner_label_end = eval_dataset.ner_label_list[ner_preds2[i, k]]
                    # print((float(ner_values2[i, k]), ner_label_end,end))
                    if ner_label_end != 'NIL':
                        scores_end.append((float(ner_values2[i, k]), ner_label_end,end))

                scores_start.sort(key=lambda x: -x[0])
                scores_end.sort(key=lambda x: -x[0])
                hash_map = {}
                for scores_start_i in scores_start:
                    result = hash_map.get(scores_start_i[2], 0)
                    if result==0:
                        hash_map[scores_start_i[2]] = 1
                        scores_new_start.append(scores_start_i)

                hash_map = {}
                for scores_end_i in scores_end:
                    result = hash_map.get(scores_end_i[2], 0)
                    if result==0:
                        hash_map[scores_end_i[2]] = 1
                        scores_new_end.append(scores_end_i)

                scores_new_start.sort(key=lambda x: -x[2])
                scores_new_end.sort(key=lambda x: -x[2])

                # print('scores_start',scores_start)
                # print('scores_end', scores_end)

                for index_i, scores_i in enumerate(scores_start):
                    if index_i!= len(scores_start)-1:
                        next_scores_i = scores_start[index_i+1]
                    else:
                        next_scores_i = []
                    for scores_j in scores_end:
                        if next_scores_i:
                            if scores_j[2] > next_scores_i[2]:
                                break
                        if scores_j[2]<scores_i[2]:
                            continue
                        if scores_i[1] == scores_j[1]:
                            scores[(index[0], index[1])][(scores_i[2], scores_j[2])] = (scores_i[0]+scores_j[0], scores_j[1])
...
def main():
    ...
    parser.add_argument("--output_dir", default='ace05ner_models/PL-Marker-ace05-bert_attention', type=str,
    ...
    parser.add_argument('--max_pair_length', type=int, default=60, help="")
    ...
    if args.do_train and args.local_rank in [-1, 0] and args.output_dir.find('test') == -1:
        create_exp_dir(args.output_dir,
                       scripts_to_save=['run_ner.py', 'models.py',])
    ...
    if args.model_type.startswith('albert'):
        config_class, model_class, tokenizer_class = AlbertConfig,  BertForBinarySpanMarkerNER, AlbertTokenizer
    else:
        config_class, model_class, tokenizer_class = BertConfig, BertForBinarySpanMarkerNER, BertTokenizer

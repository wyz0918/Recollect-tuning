...
from models import BertForACEBothOneDropoutSub6, BertForACEBothOneDropoutSubNoNer, AlbertForACEBothOneDropoutSub
...
class ACEDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i_index, i_input in enumerate(input_ids):
            # if i_input==30000 or i_input==30001:
        ...
        input_mask = [1] * L + [0] * (self.max_seq_length - L) + [0] * (self.max_entity_length) * 3
        ...
        position_ids = list(range(sub_position[0])) + [sub_position[0]] + [sub_position[0]] + list(
            range(sub_position[0], sub_position[1] - 4)) + [sub_position[1] - 4] + [sub_position[1] - 4] + list(
            range(sub_position[1] - 4, L - 4)) + [0] * (self.max_seq_length - L) + [0] * self.max_entity_length * 3

        start = int(sub_position[0])
        end = int(sub_position[1])
        middle = end - start

        if middle == 1 or middle == 0:
            s2 = s4 = s6 = start
            s3 = s5 = s7 = end
        elif middle == 2 or middle == 3:
            s2 = start
            s3 = end
        ...
        input_ids = input_ids + id_start2 + [self.tokenizer.pad_token_id] * (
                self.max_pair_length - len(entry['examples'])) + id_end2 + [self.tokenizer.pad_token_id] * (
                            self.max_pair_length - len(entry['examples']))

        input_ids = input_ids + id_start3 + [self.tokenizer.pad_token_id] * (
                self.max_pair_length - len(entry['examples'])) + id_end3 + [self.tokenizer.pad_token_id] * (
                            self.max_pair_length - len(entry['examples']))
        ...
        mention_pos += [(0, 0, 0, 0, 0, 0)] * (num_pair - len(mention_pos))
...
def main():
    ...
    parser.add_argument("--max_seq_length", default=182, type=int,
    ...
    parser.add_argument('--max_pair_length', type=int, default=41, help="")
    ...
    if args.do_train and args.local_rank in [-1, 0] and args.output_dir.find('test') == -1:
        create_exp_dir(args.output_dir, scripts_to_save=['run_re6.py', 'models.py'])
    ...
    if args.model_type.startswith('bert'):
        config_class, model_class, tokenizer_class = BertConfig, BertForACEBothOneDropoutSub6, BertTokenizer
    else:
        config_class, model_class, tokenizer_class = AlbertConfig, AlbertForACEBothOneDropoutSub, AlbertTokenizer

...
from models import BertForACEBothOneDropoutSub8, BertForACEBothOneDropoutSubNoNer, AlbertForACEBothOneDropoutSub
...
class ACEDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        input_mask = [1] * L + [0] * (self.max_seq_length - L) + [0] * (self.max_entity_length) * 4
        ...
        position_ids = list(range(sub_position[0])) + [sub_position[0]] + [sub_position[0]] + list(
            ...
            range(sub_position[1] - 4, L - 4)) + [0] * (self.max_seq_length - L) + [0] * self.max_entity_length * 4
        ...
        for x_idx, obj in enumerate(entry['examples']):
            ...
            mention_pos.append((pos_index[0], pos_index[1], pos_index[2], pos_index[3],pos_index[4], pos_index[5],pos_index[6], pos_index[7]))
        ...
        mention_pos += [(0, 0, 0, 0, 0, 0,0,0)] * (num_pair - len(mention_pos))
...
def main():
    ...
    if args.do_train and args.local_rank in [-1, 0] and args.output_dir.find('test') == -1:
        create_exp_dir(args.output_dir, scripts_to_save=['run_re8.py', 'models.py'])
    ...
    if args.model_type.startswith('bert'):
        config_class, model_class, tokenizer_class = BertConfig, BertForACEBothOneDropoutSub8, BertTokenizer
    else:

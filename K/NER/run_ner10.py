...
from models import BertForSpanMarkerNER10, AlbertForSpanMarkerNER
    ...
    def __getitem__(self, idx):
        ...
        for x_idx, x in enumerate(entry['examples']):
            ...
            mention_pos.append((pos_index[0], pos_index[1], pos_index[2], pos_index[3], pos_index[4], pos_index[5],
                                pos_index[6], pos_index[7],pos_index[8], pos_index[9]))            # boundary_width.append(m1[1]-m1[0]+1)
...
def evaluate(args, model, tokenizer, prefix="", do_test=False):
    ...
    predict_ners2 = defaultdict(list)
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            ner_values2, ner_preds2 = torch.topk(ner_logits, k=2, dim=-1, largest=True)
            ...
            for i in range(len(indexs)):
                ...
                for j in range(len(m2s)):
                    ...
                    if ner_label != 'NIL' and float(ner_values[i, j])>0.7:
                        scores[(index[0], index[1])][(obj[0], obj[1])] = (float(ner_values[i, j]), ner_label)
                    elif float(ner_values2[i, j][0])-float(ner_values2[i, j][1])<0.2 or float(ner_values[i, j])<=0.5:
                        ner_label2 = eval_dataset.ner_label_list[ner_preds2[i, j][1]]
                        scores2[(index[0], index[1])][(obj[0], obj[1])] = (float(ner_values[i, j]), ner_label)
    ...
    for example_index, pair_dict in scores2.items():
        ...
        for item in no_overlap:
            ...
            tot_pred += 1
            if args.output_results:
                predict_ners2[example_index].append((m2[0], m2[1], pred_ner_label))
    ...
    if args.output_results and (do_test or not args.do_train):
        ...
        for l_idx, line in enumerate(f):
            ...
            for n in range(num_sents):
                ...
                item2 = predict_ners2.get((l_idx, n), [])
                item2.sort()

                predicted_ner.append(item)
                uncertain_ner.append(item2)

            data['predicted_ner'] = predicted_ner

            data['uncertain_ner'] = uncertain_ner
...
def main():
    ...
    if args.do_train and args.local_rank in [-1, 0] and args.output_dir.find('test') == -1:
        create_exp_dir(args.output_dir,
                       scripts_to_save=['run_ner10.py', 'models.py',])
    ...
    if args.model_type.startswith('albert'):
        ...
        config_class, model_class, tokenizer_class = BertConfig, BertForSpanMarkerNER10, BertTokenizer

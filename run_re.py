...
from models import BertForACEBothOneDropoutSub2, BertForACEBothOneDropoutSubNoNer, AlbertForACEBothOneDropoutSub
...
class ACEDataset(Dataset):
        ...
        if not evaluate:
            ...
            if do_test:
                ...
                if args.dev_file.find('models') == -1:
                    file_path = os.path.join(args.dev_file)
                else:
    ...
    def initialize(self):
        ...
        for l_idx, line in enumerate(f):
            ...
            #     p_ners = data['predicted_ner']
            #     tmp_ners = data['ner']
            #     ners = []
            #
            #
            #     for i,ner in enumerate(p_ners):
            #         if ner :
            #             new_list =[]
            #             for ner_i in ner:
            #                 if ner_i in tmp_ners[i]:
            #                     new_list.append(ner_i)
            #             ners.append(new_list)
            #         else:
            #             ners.append([])
            #
    ...
    def __getitem__(self, idx):
        ...
        input_mask = [1] * L + [0] * (self.max_seq_length - L) + [0] * (self.max_entity_length) * 2
        ...
        position_ids = list(range(sub_position[0])) + [sub_position[0]] + [sub_position[0]] + list(
            ...
            range(sub_position[1] - 4, L - 4)) + [0] * (self.max_seq_length - L) + [0] * self.max_entity_length * 2
        ...
        for x_idx, obj in enumerate(entry['examples']):
            ...
            mention_pos.append((m1[0], m1[1], m2, m3))

            # mention_pos.append((m1[0], m1[1], m2, m3, m4, m5, m6, m7))
            ...
            id_start2 += [label2ids[sub_label]]
            id_end2 += [label2ids[sub_label]]
        ...
        mention_pos += [(0, 0, 0, 0)] * (num_pair - len(mention_pos))
...
def evaluate(args, model, tokenizer, prefix="", do_test=False):
    ...
    json.dump(results, open(f1_output_dir, "a"))
...
def main():
    ...
    if args.do_train and args.local_rank in [-1, 0] and args.output_dir.find('test') == -1:
        create_exp_dir(args.output_dir, scripts_to_save=['run_re.py', 'models.py'])
    ...
    if args.model_type.startswith('bert'):
        config_class, model_class, tokenizer_class = BertConfig, BertForACEBothOneDropoutSub2, BertTokenizer
    else:
